...
// 内嵌的「模板字段展开行」已移除：异步加载存在「加载中…卡住」与
// 「父行被过滤后展开行残留」等竞态问题；模板字段统一通过双击/编辑
// 对话框查看，避免与 naive-ui DataTable expand 的状态耦合。
...
const columns = computed<DataTableColumns<PasswordSummary>>(() => {
  ...
  // 按用户偏好过滤列；分类列动态宽度
  return allColumns
    .filter((col: any) => visibleColumnKeys.value.includes(col.key))
